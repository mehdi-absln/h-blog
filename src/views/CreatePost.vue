<template>
  <div class="bg-light editor-wrapper">
    ...
    <div class="container py-3">
      ...
      <button @click="uploadBlog" class="btn btn-danger text-capitalize py-2 px-4 rounded-pill text-white mt-3">publish
        blog
      </button>
      <router-link to="/post-preview"
                   class="btn btn-danger text-capitalize py-2 px-4 rounded-pill text-white mt-3 ms-1">post preview
      </router-link>
    </div>
  </div>
</template>
    ...
    uploadBlog() {
      if (this.blogTitle.length !== 0 && this.blogHtml !== "<p><br></p>") {
        if (this.file) {
          ...
          this.error = true;
          this.errorMsg = "please upload cover photo";
          setTimeout(() => {
            this.error = false;
          }, 5000)
        }
